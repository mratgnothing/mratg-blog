...
const out='output/playwright/gaoxiao-online';await fs.mkdir(out,{recursive:true});
...
try {
  ...
  assert.equal(await a.locator('.gxf-player').count(),4);
  assert.equal(await a.locator('.gxf-player header span').filter({hasText:'真人玩家'}).count(),2);
  ...
  await a.screenshot({path:`${out}/desktop-room.png`,fullPage:true});
  ...
  await c.locator('#practice').click();await c.locator('#room:not([hidden])').waitFor({timeout:10000});assert.equal(await c.locator('.gxf-player').count(),5);assert.equal(await c.locator('.gxf-player header span').filter({hasText:'AI 玩家'}).count(),4);
  evidence.push('Practice immediately started a 5-seat game with 1 human + 4 AI.');
